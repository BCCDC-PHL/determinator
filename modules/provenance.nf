process collect_provenance {
  tag { sample_id }
  publishDir "${params.outdir}/provenance/${sample_id}", pattern: "${sample_id}_*_provenance.yml", mode: 'copy'
  input:
...
process pipeline_provenance {
  ...
  executor 'local'
  publishDir "${params.outdir}/pipeline_info", pattern: "pipeline_provenance.yml", mode: 'copy'
